...
-   **Housekeeping (indexes + admin-alert env var + weekly-push split, partial)**: (1) Added three single-column indexes on `schedules` — `idx_schedules_date`, `idx_schedules_coach_name`, `idx_schedules_coach_name_2` — via `index()` API in `shared/schema.ts` and pushed with `npm run db:push`. Verified in PG. (2) Removed hardcoded `CHEN_BO_RONG_LINE_ID` constant from `server/modules/coachPortal.routes.ts`; replaced with new env var `ADMIN_ALERT_LINE_USER_ID` exposed as `env.adminAlertLineUserId` in `server/config/env.ts`. When unset, the LINE notification is downgraded to a `console.warn` instead of pushing to the stale id. (3) Split `client/src/pages/weekly-push.tsx` (720 LOC) by extracting `RunDetailDialog` and shared formatting helpers (`fmtDate`, `fmtWeek`, `statusBadge`, `recipientStatusBadge`, `triggerLabel`, `adminPassword`) into `client/src/components/weekly-push/RunDetailDialog.tsx` (330) + `helpers.tsx` (127). Page is now 291 LOC. **Deferred**: splitting `coach-approval.tsx` (1516), `coach-portal.tsx` (1059), and `coach-assignment.tsx` (1019) — each needs multiple component extractions (~3-5) against shipped admin/coach UI with real regression risk; a separate scoped task should drive those.
-   **Multi-school Schema-Name Safety (Task #33, partial)**: Removed all `sql.raw` schema-name interpolation in the multi-school code path (`server/multi-school-db.ts`, `server/modules/school.repo.ts`). Centralized two helpers in `server/multi-school-db.ts`: `assertSchoolCode(code)` (regex `/^[a-zA-Z0-9_]+$/` + runtime whitelist `getAvailableSchools()`) and `schoolSchemaName(code)` returning the validated `school_<code>` string. All identifier interpolation now goes through `sql.identifier(schoolSchemaName(code))`, so the schema name is both validated AND quoted by drizzle. Demo seed-data INSERTs also switched from inline string templating to bound `sql` template parameters. Verified live: all three schemas (demo/school1/school2) init cleanly on boot; `/api/demo/teachers` and `/api/demo/schedules` return data through the new path. **Deferred to follow-up Task #40**: extracting a unified schedule/feedback service layer that both the master and multi-school routes share — that piece would touch `server/repositories/schedule.repository.ts` (469 LOC) and `server/modules/schedule.routes.ts` (319 LOC), and the only concrete shared feature listed in the task description (`isClassLocked`) is already present in the multi-school schema (`is_class_locked` column added in the schema-init `ALTER TABLE` block).
-   **Outbound HTTP Timeouts (Task #32)**: All outbound HTTP calls (LINE Messaging API push, LINE OAuth token + profile, Ragic data sync) now go through `server/shared/http/fetchWithTimeout.ts`, which wraps `fetch` with `AbortController` and a default 8s timeout (per-call override via `timeoutMs`, env override `OUTBOUND_HTTP_TIMEOUT_MS`). Returns a structured `{ok, status, body, errorCode, errorMessage}` with classified `errorCode: timeout|network|http_4xx|http_5xx`. Ragic uses 20s (heavy queries). Replaced bare `fetch` in: `server/line-notify.ts`, `server/modules/notification/lineNotify.adapter.ts`, `server/modules/coachPortal.routes.ts` (LINE notify-陳柏榮), `server/modules/auth.routes.ts` (LINE OAuth token + profile), `server/modules/notify.routes.ts` (manual LINE re-push), `server/ragic.ts` (Ragic API). Out of scope (already bounded / Replit-managed): `server/infra/healthchecks/healthchecks.client.ts` (5s AbortController), `server/replit_integrations/object_storage/objectStorage.ts` (sidecar, local). **Worker retry policy updated**: `server/modules/weeklyPush/weeklyPush.worker.ts` now treats `errorCode==='http_4xx' && status!==429` as a hard final failure (no retry) — invalid `to`, blocked user, expired token won't change on retry. 429 (rate limit) and 5xx/timeout/network remain transient (pg-boss retries). Smoke `npx tsx scripts/smoke-weekly-push.ts` dry-run pipeline still passes (recipient send path, report generation).
-   **Report Cleanup + Excel Export**: (1) 90-day automatic report cleanup — `server/modules/weeklyPush/weeklyPush.cleanup.ts` runs daily at 03:30 Asia/Taipei (gated by `enableWeeklyPushQueue`); queries `weekly_push_runs` for rows older than 90 days with a non-null `report_path`, deletes from Object Storage via new `deleteReport()` in `reportStorage.ts`, and nulls out the column. (2) Excel (XLSX) report download — `exceljs` installed; `renderXlsx()` added to `weeklyPush.report.ts`; GET `/api/admin/weekly-push/runs/:runId/report?format=xlsx` generates on demand from DB (no extra storage). CSV remains the stored/primary format. `weeklyPushRepo.listRunsWithExpiredReports(cutoff)` added to repository.
-   **Push Pipeline Hardening**: Added `weeklyLogAlreadyExists()` to prevent duplicate weekly push when cron + manual trigger fire same Sunday. Added per-coach `try/catch` in both daily and weekly push loops (one coach failure no longer aborts others). Both cron callbacks now have `.catch()` to prevent unhandled promise rejections.
